...
void LoadCascade(DetectionScanner& ds)
{
    ...
    filenames.push_back("/home/rover/catkin_ws/src/hdetection/src/combined.txt.model");
    types.push_back(NodeDetector::CD_HIK); // second node
    upper_bounds.push_back(353);
    filenames.push_back("/home/rover/catkin_ws/src/hdetection/src/combined.txt.model_");
...
CRect PDetection(cv::Mat src, float fx){
    LoadCascade(scanner);
    bool rect_organization = true; //reorganize the rects
    IntImage<double> original;

    if (fx < 1)
    {
        cv::resize(src, src, cv::Size(), fx, fx); //change the size of image
    }
        
    original.Load(src);
    std::vector<CRect> results;
    scanner.FastScan(original, results, 2); //fast scan

    if(rect_organization)
    {
        PostProcess(results,2); //read the code
        PostProcess(results,0);
        RemoveCoveredRectangles(results);
    }

    //add the filter in here
    double MaxSize = 0;
    int MaxSizeIndex = 0;
    ...
    cv::rectangle(src, cvPoint(results[MaxSizeIndex].left,results[MaxSizeIndex].top),cvPoint(results[MaxSizeIndex].right,results[MaxSizeIndex].bottom),cv::Scalar(0,255,0),2 );
    CRect LResult = results[MaxSizeIndex];
    //std::cout << LResult;
    return LResult;
}    
...
{    
    ...
    ImageConvertor():it_(nh_){    
        ...
        cv::namedWindow("B_OUT_WINDOW", CV_WINDOW_AUTOSIZE);    
        cv::namedWindow("B_IN_WINDOW", CV_WINDOW_NORMAL);       
    }    
    
    ~ImageConvertor()    
    {    
        cv::destroyWindow("B_OUT_WINDOW");    
        cv::destroyWindow("B_IN_WINDOW");    
    }    
    ...
    {    
        ...
        {    
            /*转化成CVImage*/    
             ...
             cv::imshow("B_IN_WINDOW",cv_ptr->image);  
             cv::waitKey(30);  
        }    
        ...
        cv::Mat img_out;
        CRect RoiRect;
        RoiRect = PDetection(cv_ptr->image,1);
        img_out = cv_ptr->image(cv::Rect(RoiRect.left,RoiRect.top,(RoiRect.right - RoiRect.left),(RoiRect.bottom - RoiRect.top)));    

        cv::imshow("B_OUT_WINDOW",img_out);  
        cv::waitKey(3);
        sensor_msgs::ImagePtr msgnew;
        msgnew = cv_bridge::CvImage(std_msgs::Header(), "bgr8", img_out).toImageMsg();  
        image_pub_.publish(msgnew);        
    }    
